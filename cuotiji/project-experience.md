...
## 4. PromptX 角色“户籍”管理（2025-11-27）

- **错误描述**：
  - 角色文件虽然存在于项目文件夹的 `.promptx/resource/xiaoli` 中，但在 PromptX 客户端（Discover）中却看不见，或者重启后消失。
  - 误以为只要项目里有文件，PromptX 全局就能自动识别。

- **错误本质**：
  - **作用域混淆**：
    - **项目角色 (Project Scope)**：位于项目的 `.promptx` 目录。必须执行 `project` 绑定指令，且随项目环境变动，不稳定。
    - **用户角色 (User Scope)**：位于用户主目录 `~/.promptx/resource/role`（Windows下是 `C:\Users\User\.promptx`）。这是全局永久生效的“本地户口”。

- **正确解法（SOP）**：
  1.  **全局定居**：若要让角色成为跨项目的“随身助手”，直接将其文件复制到 **用户全局目录**（`C:\Users\username\.promptx\resource\role\角色名`）。
  2.  **源码归源码**：项目里的源码目录（如 `role/xiaoli`）仅作为 Git 版本控制和编辑源，不直接作为 PromptX 的运行库。

- **核心原则**：
  - **想永久，进 User**：不要在项目临时目录里纠缠，直接进系统级目录。
